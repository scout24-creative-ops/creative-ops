...
- The current Contentful Module Library page is a stored `lpBuilder` HTML snapshot in `htmlSourcePayload`, not a dynamic sync from the repository. The maintained capability source is the Contentful Builder `gpt-package/component-library.html`; `libraries/module-library.html` is generated locally from its marked module/component blocks but is not automatically connected to the Contentful page.
